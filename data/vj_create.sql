...
CREATE TABLE `vj_cdk` (
  ...
  KEY `use_id` (`use_id`)
) ENGINE=InnoD DEFAULT CHARSET=utf8 COMMENT='卡密充值表';
ALTER table `vj_cdk` add `classify` VARCHAR(255) NOT NULL DEFAULT 'default' COMMENT '卡密分类';
...
ALTER table `vj_user` add `classify` VARCHAR(255) NOT NULL DEFAULT 'default' COMMENT '会员分类';
-- ----------------------------
...
ALTER TABLE `vj_video` DROP `id`;
ALTER TABLE `vj_video` ADD `id` int(11) PRIMARY KEY NOT NULL AUTO_INCREMENT COMMENT '自增ID' FIRST ;
-- ----------------------------
...
ALTER TABLE `vj_video_coll` add `digest` tinyint(1) NOT NULL DEFAULT 0 COMMENT '精品视频: 1-是;0-不是';
